...
set -e
cwd=`pwd`
killall boot3-x86_64-static || true
sleep 3
cp -p boot3-x86_64-static.exe ~/../
rm -rf *.tmp
rm -rf common
cp -rp ~/qt/common .
...
ts=`date "+%Y.%m.%d.%H.%M.%S"`
echo $ts > main/spider3.version.txt
UNAME=`uname`
...
cat << EOS > spider.json
{
    ...
    "url": [
        "https://github.com/spider-explorer/spider3/releases/download/v$ts/spider-v$ts.zip"
    ],
    "hash": [
        "$sum1"
    ],
